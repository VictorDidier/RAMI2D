...
class Image:
    ...
    def __init__(self,image_file,mpp):
        """
    ...
    def get_pyramidal_level(self,level=0,ch=0):
        if self.is_slide:
            ...
            return self.array_level(level,ch)
        else:
            raise ValueError("Unknown data type")
    ...
    def array_level(self,level,ch=0):
        path=self.file
    ...
    def slide_level(self,level):
        ...
        if self.props["color_type"]=="RGBA":
            ...
            level_array = Image._convert_rgba_to_rgb(level_array, self.props["background"])
