...
import processing_tools as prt
from processing_tools import Image
...
def apply_transform(img_object,transform_map,mpp_full):
...
def create_pyramid(img_instances,
    ...
    for INST,TYPE,LEVL in zip(img_instances,types,pyramid_levels):
        ...
        if TYPE=="generator":
            for channel in INST:
                unfolded_instances.append( pyramid_gaussian( channel  , max_layer=levels-1, preserve_range=True,order=1,sigma=1,channel_axis=2) )
...
def main(version):
    ...
    Mov=Image(moving_img_path,mpp_mov)
    ...
    tifff.imwrite(qc_dir/"fixed.tif",reg_fixed_img,photometric="minisblack")
    tifff.imwrite(qc_dir/"moving.tif",reg_moving_img,photometric="minisblack")

    #Calculate initial alignment
    if apply_initial_alignment:
        init_trf_dir=out_trf_dir / "00_initial"
        ...
        tifff.imwrite(qc_dir/"moving_init_aligned.tif",reg_moving_img,photometric="minisblack")
    else:
        init_trf=None
    ...
    if not test_mode:
        ...
        moving_props_out=Image(out_img_path,out_mpp).props
        if markers:
