...
def naive_descriptor_matching(fix_arr,mov_arr):
    ...
    matches=match_descriptors(descriptors[0],
                              ...
                              max_ratio=0.6,
                              cross_check=False)
    print("Number of keypoints found by SIFT:",len(matches))
...
def create_trf_object(center_of_rotation,displacement,theta,fliph,mpp,output_size):
    ...
    # this is only implemented when the user requested a flip in the
    # get_initial_trf function.
    #-------------------------------------------------
    if fliph:
        image_center=[(output_size[0]/2)*mpp,(output_size[1]/2)*mpp]
        trf_object.AddParameterFile(str(flip_template))
        trf_object.SetParameter(map_counter,"CenterOfRotationPoint",[str(val) for val in image_center ])
        trf_object.SetParameter(map_counter,"Size", [str(val) for val in output_size ])
        trf_object.SetParameter(map_counter,"Spacing",[str(mpp),str(mpp)])
        trf_object.SetParameter(map_counter,"Origin",["0","0"])
        map_counter+=1
    #------------------------------------------------
    ...
    trf_object.SetParameter(map_counter,"CenterOfRotationPoint",[str(val) for val in center_of_rotation ])
    trf_object.SetParameter(map_counter,"TransformParameters", [ str(theta),"0", "0"])#theta,x,y [pos-counterclockwise/neg-clockwise,neg-right/pos-left,neg-down/pos-up]
    trf_object.SetParameter(map_counter,"Size",[str(val) for val in output_size ])
    trf_object.SetParameter(map_counter,"Spacing",[str(mpp),str(mpp)])
    trf_object.SetParameter(map_counter,"Origin",["0","0"])
    map_counter+=1
    ...
    trf_object.SetParameter(map_counter,"TransformParameters", [str(val) for val in displacement ])#theta,x,y [pos-counterclockwise/neg-clockwise,neg-right/pos-left,neg-down/pos-up]
    trf_object.SetParameter(map_counter,"Size", [str(val) for val in output_size ])
    trf_object.SetParameter(map_counter,"Spacing",[str(mpp),str(mpp)])
    trf_object.SetParameter(map_counter,"Origin",["0","0"])
    return trf_object

def get_initial_trf(fix_arr,mov_arr,input_mpp,qc_dir,output_dict=None,fliph_mov=False,file_extension=".png"):
    if not output_dict:
    ...
    if fliph_mov:
        #horizontal flip to moving image
        mov_arr=np.flip(mov_arr,axis=1)
    ...
    init_trf=create_trf_object(center_of_rotation,translation,theta,fliph_mov,output_mpp,output_dims)
