...
def CHANN_array(no_of_channels, inputs):
    ...
    CHANN = [
        Channel(
            ...
            samples_per_pixel=1 if inputs["color"][ch]=="grayscale" else 3
            )
        for ch in range(0,no_of_channels)
        ]
...
def PIXELS_array(chann_block, plane_block, tiff_block, inputs):
    ...
    PIXELS = Pixels(
        ...
        size_c=len(chann_block) if inputs["color"][0]=="grayscale" else 3,
        size_t=1,
        ...
        interleaved=False if inputs["color"][0]=="grayscale" else True,
